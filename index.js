...
cron.schedule('0 9  * * *', () => postArticle(0),   { timezone: TIMEZONE }); // 9:00 AM article 1
cron.schedule('0 10 * * *', () => postArticle(1),   { timezone: TIMEZONE }); // 10:00 AM article 2
cron.schedule('0 11 * * *', () => postArticle(2),   { timezone: TIMEZONE }); // 11:00 AM article 3
cron.schedule('0 12 * * *', () => postArticle(3),   { timezone: TIMEZONE }); // 12:00 PM article 4
cron.schedule('0 13 * * *', () => postArticle(4),   { timezone: TIMEZONE }); // 1:00 PM article 5
// ============================================================

logger.info('\n📅  Schedule (Dubai Time / UTC+4):');
logger.info('     6:05 AM  → Scrape dubai5.space (with retries)');
logger.info('     9:00 AM  → Post Article 1 → All platforms');
logger.info('    10:00 AM  → Post Article 2 → All platforms');
logger.info('    11:00 AM  → Post Article 3 → All platforms');
logger.info('    12:00 PM  → Post Article 4 → All platforms');
logger.info('     1:00 PM  → Post Article 5 → All platforms');
